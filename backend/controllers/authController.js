...
const loginUser = async (req, res) => {
  ...
  try {
    const user = await pool.query('SELECT id, email, password_hash, role, is_premium FROM users WHERE email = $1', [email]);
    ...
    res.json({ token, user: { id: dbUser.id, email, role: dbUser.role, fullName, is_premium: dbUser.is_premium } });
  } catch (err) {
...
const googleLogin = async (req, res) => {
  ...
  try {
    ...
    res.json({ token: appToken, user: { id: dbUser.id, email, role: dbUser.role, fullName, is_premium: dbUser.is_premium } });
  } catch (err) {
...
const forgotPassword = async (req, res) => {
  ...
  try {
    ...
    // Ensure column names reset_otp and reset_otp_expiry exist in your DB
    await pool.query(
...
const resetPassword = async (req, res) => {
  ...
  try {
    ...
    // Crucial: Update the correct column 'password_hash'
