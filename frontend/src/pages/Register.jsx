...
import { AuthContext } from '../context/AuthContext'; // Added context for theme
import { GoogleLogin } from '@react-oauth/google';
import { User, Mail, Lock, UserCircle, ArrowRight, ShieldCheck, Gift, Loader2, Stethoscope, Sparkles } from 'lucide-react';
import { toast } from 'react-toastify';
import { motion } from 'framer-motion';

const Register = () => {
  const { theme } = useContext(AuthContext); // Access global theme
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 font-sans transition-colors duration-500 ${isPremium ? 'bg-slate-950' : 'bg-slate-50'}`}>
      ...
      >
        ...
        {/* Right Side: Form (All original logic included) */}
        <div className="md:w-2/3 p-10 md:p-12">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            {/* ROLE SELECTION CARDS (Added professional selection) */}
            ...
            <button
              type="submit"
              disabled={loading}
              className={`w-full py-4 px-4 rounded-xl font-bold flex justify-center items-center gap-2 transition-all shadow-xl ${
                formData.role === 'doctor' 
...
// Helper Component for Inputs (Updated to be theme-aware)
const InputGroup = ({ icon, isPremium, ...props }) => (
  <div className="relative group text-left">
    ...
    <input
      {...props}
      className={`block w-full pl-12 pr-4 py-4 rounded-xl transition-all font-medium border-0 outline-none ${
        isPremium 
