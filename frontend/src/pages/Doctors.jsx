...
const Doctors = () => {
  ...
  const cardClass = isPremium 
    ? "bg-slate-900 border-yellow-500/10 shadow-2xl hover:border-yellow-500/40" 
  ...
  const headerGradient = isPremium 
    ? "from-yellow-900/40 to-slate-900" 
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="text-center mb-16">
          <div className={`inline-flex items-center gap-2 px-4 py-1 rounded-full text-xs font-bold uppercase tracking-widest mb-4 
            ${isPremium ? 'bg-yellow-500/10 text-yellow-500 border border-yellow-500/20' : 'bg-emerald-100 text-emerald-800'}`}>
          ...
          <div className="mt-8 max-w-lg mx-auto relative group">
            <Search className={`absolute left-4 top-4 h-5 w-5 transition-colors 
              ${isPremium ? 'text-yellow-500/50 group-focus-within:text-yellow-500' : 'text-slate-400 group-focus-within:text-emerald-600'}`} 
            />
            <input 
              ...
              className={`w-full pl-12 pr-4 py-4 rounded-2xl outline-none transition-all border
                ${isPremium 
                  ? 'bg-slate-900 border-yellow-500/10 text-white focus:ring-2 focus:ring-yellow-500/20 shadow-none' 
                  : 'bg-white border-transparent shadow-xl shadow-slate-200/50 focus:ring-2 focus:ring-emerald-500/20'}`}
            />
          </div>
        </motion.div>
              ...
              >
                ...
                <div className={`h-28 bg-gradient-to-br relative ${headerGradient}`}>
                    ...
                    <div className={`absolute top-4 right-4 backdrop-blur-md px-3 py-1 rounded-full flex items-center gap-1 border 
                      ${isPremium ? 'bg-yellow-500/20 border-yellow-500/30' : 'bg-white/10 border-white/20'}`}>
                ...
                <div className="p-6 relative">
                    ...
                    <div className={`absolute -top-12 left-6 h-20 w-20 rounded-2xl p-1 shadow-lg transform group-hover:scale-105 transition-transform
                      ${isPremium ? 'bg-slate-900' : 'bg-white'}`}>
                        <div className={`h-full w-full rounded-xl flex items-center justify-center text-2xl font-serif font-bold 
                          ${isPremium ? 'bg-slate-800 text-yellow-500 border border-yellow-500/20' : 'bg-slate-100 text-slate-400'}`}>
                            {doc.full_name.charAt(0)}
                        </div>
                    </div>
                    
                    <div className="mt-8 space-y-1">
                        <p className={`text-xs font-black uppercase tracking-widest ${isPremium ? 'text-yellow-500' : 'text-emerald-600'}`}>
                          {doc.specialization}
                        </p>
                    ...
                    <div className="mt-6 space-y-3">
                        <div className={`flex items-center gap-3 p-3 rounded-xl border transition-colors
                          ${isPremium ? 'bg-slate-800/50 border-yellow-500/5' : 'bg-slate-50 border-slate-100'}`}>
                            <Clock className={`h-4 w-4 ${isPremium ? 'text-yellow-500' : 'text-emerald-600'}`} />
                            <span className={`text-sm font-medium ${isPremium ? 'text-slate-300' : 'text-slate-700'}`}>
                    ...
                    <motion.button 
                        ...
                        className={`w-full mt-6 py-4 rounded-xl font-bold text-sm tracking-wide transition-all shadow-lg flex items-center justify-center gap-2
                          ${!user 
                            ? (isPremium ? 'bg-white text-slate-950 hover:bg-yellow-500' : 'bg-slate-900 text-white hover:bg-black') 
                            : (isPremium ? 'bg-yellow-500 text-slate-950 hover:bg-yellow-400' : 'bg-emerald-600 text-white hover:bg-emerald-700 shadow-emerald-500/30')
                          }`}
                    >
