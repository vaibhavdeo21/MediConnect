...
import { User, Mail, Phone, MapPin, Briefcase, IndianRupee, Clock, Calendar, Save, Sparkles, ChevronDown, X } from 'lucide-react';
...
const Profile = () => {
    ...
    const cardClass = isDark ? "bg-slate-900 border-white/5 shadow-2xl" : "bg-white border-blue-100 shadow-xl shadow-blue-900/5";
    ...
    const renderInput = (label, name, icon, type = "text", readOnly = false) => (
        <div className="space-y-2">
            ...
            <div 
                ...
                onClick={() => !readOnly && setActiveField({ label, name, icon, type })}
            >
                <div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-hover:text-blue-500 transition-colors">
                    {icon}
                </div>
                {/* FIXED: Ensure Contact Number visibility by passing value correctly */}
                <input 
                    name={name} 
                    value={formData[name] || ''} 
                    readOnly 
                    placeholder={`No ${label.toLowerCase()} set`}
                    className={`w-full pl-12 pr-4 py-4 rounded-2xl outline-none border transition-all pointer-events-none ${inputClass} ${readOnly ? 'opacity-50 grayscale' : ''}`} 
                />
            </div>
        </div>
    );
    ...
    return (
        <div className={`min-h-screen py-24 px-4 transition-colors duration-500 ${bgClass} relative`}>
            {/* FOCUS MODE MODAL - FIXED CENTER POSITIONING */}
            <AnimatePresence>
                {activeField && (
                    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
                        ...
                        <motion.div 
                            ...
                            className={`relative z-[1001] w-full max-w-lg p-10 rounded-[3rem] border shadow-2xl ${cardClass}`}
                        >
                            <div className="flex justify-between items-center mb-8">
                                <h3 className="text-2xl font-serif font-bold italic">Editing {activeField.label}</h3>
                            ...
                            <div className="relative mb-8">
                                <div className="absolute left-5 top-1/2 -translate-y-1/2 text-blue-500 h-6 w-6">{activeField.icon}</div>
                                <input 
                                    autoFocus
                                    ...
                                    className={`w-full pl-14 pr-6 py-5 rounded-2xl outline-none border-2 border-blue-500 shadow-xl shadow-blue-500/10 text-lg font-medium ${inputClass}`}
                                />
                            </div>
                            <button 
                                onClick={() => setActiveField(null)}
                                className="w-full py-5 bg-blue-600 text-white text-lg font-bold rounded-2xl shadow-xl hover:bg-blue-500 transition-all active:scale-[0.98]"
                            >
                                Done
                            </button>
                        </motion.div>
            ...
            <motion.div className={`max-w-3xl mx-auto rounded-[2.5rem] overflow-hidden border ${cardClass}`}>
                {/* UPDATED HEADER: Now reacts to theme */}
                <div className={`px-8 py-10 flex items-center justify-between transition-colors duration-500 ${isDark ? 'bg-slate-900 border-b border-white/5 shadow-inner' : 'bg-slate-900 text-white'}`}>
                    <div className="text-left">
                        <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-[10px] uppercase font-bold tracking-widest mb-3 ${
                            ...
                            isPremium ? 'bg-yellow-500/20 text-yellow-500 border border-yellow-500/30' : 
                            'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30'
                        }`}>
                            {isDoctor ? 'Practitioner Account' : isPremium ? 'Elite Account' : 'Standard Account'}
                        </div>
                        <h1 className="text-3xl font-serif font-bold text-white">{isDoctor ? 'Practice Settings' : 'My Profile'}</h1>
                    </div>
                    <div className={`p-4 rounded-2xl ${isDoctor ? 'bg-blue-600 text-white' : isPremium ? 'bg-yellow-500 text-slate-950' : 'bg-emerald-600 text-white'}`}>
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-8 text-left">
                    ...
                    {isDoctor ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="space-y-2">
                                <label className="block text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">Specialization</label>
                    ...
                    <button type="submit" className={`w-full py-5 rounded-2xl font-bold flex items-center justify-center gap-3 transition-all shadow-xl ${
                        isDoctor ? 'bg-blue-600 text-white hover:bg-blue-500 shadow-blue-500/20' : 
                        isPremium ? 'bg-yellow-500 text-slate-950 hover:bg-yellow-400 shadow-yellow-500/20' : 
