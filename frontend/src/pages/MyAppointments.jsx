...
const MyAppointments = () => {
  ...
  // UPDATED: Theme variables that react to Dark/Light toggle
  const isDark = theme === 'dark';
  const bgClass = isDark ? "bg-slate-950 text-white" : "bg-slate-50 text-slate-900";
  const textPrimary = isDark ? "text-white" : "text-slate-900";
  const textSecondary = isDark ? "text-slate-400" : "text-slate-500";
  
  // CARD LOGIC: Professional clinical look for Light Mode
  const cardBg = isDark 
  ...
  if (loading) return <div className={`min-h-screen ${isDark ? 'bg-slate-950 text-cyan-400' : 'bg-slate-50 text-blue-600'} flex items-center justify-center`}>Syncing Schedule...</div>;

  return (
    <div className={`min-h-screen py-16 px-4 sm:px-6 lg:px-8 font-sans transition-colors duration-500 ${bgClass} text-left`}>
      <div className="max-w-5xl mx-auto">
        
        <div className={`flex items-end justify-between mb-10 border-b pb-6 ${isDark ? 'border-white/5' : 'border-slate-200'}`}>
            <div>
                <h1 className={`text-3xl font-serif font-bold ${textPrimary}`}>
                {isDoctor ? 'Patient Requests' : 'My Appointments'}
                </h1>
                <p className={`${textSecondary} mt-1`}>Manage your schedule and medical history.</p>
            </div>
            <div className={`px-4 py-2 rounded-full border text-sm font-medium shadow-sm ${isDark ? 'bg-slate-900 border-white/10 text-slate-300' : 'bg-white border-slate-200 text-slate-600'}`}>
                Total: {appointments.length}
            </div>
        </div>

        {appointments.length === 0 ? (
          <div className={`p-16 rounded-[2.5rem] shadow-sm text-center border ${cardBg}`}>
          ...
          <motion.div layout className="space-y-6">
            ...
            {appointments.map((appt) => (
              <motion.div 
                ...
                key={appt.id} 
                className={`p-6 rounded-[2.5rem] shadow-sm border hover:shadow-md transition-all duration-300 flex flex-col lg:flex-row justify-between lg:items-center gap-6 ${cardBg}`}
              >
                {/* Left: Info */}
                <div className="flex items-start gap-5">
                  <div className={`p-4 rounded-xl shadow-inner ${appt.status === 'Confirmed' ? (isDark ? 'bg-emerald-900/20' : 'bg-emerald-50') : (isDark ? 'bg-slate-800' : 'bg-slate-50')}`}>
                  ...
                  <div className="text-left">
                    <h3 className={`text-xl font-serif font-bold flex items-center gap-2 ${textPrimary}`}>
                      {isDoctor ? appt.patient_name : appt.doctor_name}
                      {/* VIP Badge Logic Kept */}
                    ...
                    <div className={`flex flex-wrap gap-4 mt-2 text-sm ${textSecondary}`}>
                        <span className="flex items-center gap-1.5 font-medium"><Clock className="h-4 w-4 opacity-70" /> {new Date(appt.appointment_date).toLocaleDateString()} at {appt.appointment_time}</span>
                        {user?.role === 'patient' && (
                            <span className="flex items-center gap-1.5 font-medium"><MapPin className="h-4 w-4 opacity-70" /> {appt.address || "Virtual Clinic"}</span>
                        )}
                        {isDoctor && appt.is_patient_premium && <span className="text-yellow-500 text-xs font-bold flex items-center gap-1"><Star className="h-3 w-3" /> Premium Plan</span>}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col lg:items-end gap-4">
                  <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest self-start lg:self-end ${
                  ...
                  <div className="flex flex-wrap gap-2">
                    {appt.status === 'Confirmed' && (
                        <>
                            ...
                            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition shadow-lg ${isDark ? 'bg-emerald-600 text-white hover:bg-emerald-500' : 'bg-emerald-500 text-white hover:bg-emerald-600'}`}>
                            <Video className="h-4 w-4" /> Join Call
                            </a>

                            <button onClick={() => setSelectedRecordAppt(appt)}
                            className={`flex items-center gap-2 border px-4 py-2 rounded-lg text-sm font-bold transition ${isDark ? 'bg-slate-800 border-white/5 text-slate-300 hover:bg-slate-700' : 'bg-white border-blue-100 text-slate-700 hover:bg-blue-50'}`}>
                    ...
                    {isDoctor && appt.status === 'Pending' && (
                        ...
                        <button onClick={() => handleStatusUpdate(appt.id, 'Confirmed')}
                            className="flex items-center gap-2 bg-emerald-500 text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-emerald-600 transition shadow-lg shadow-emerald-900/10">
                            <CheckCircle className="h-4 w-4" /> Accept
                        </button>
                        <button onClick={() => handleStatusUpdate(appt.id, 'Cancelled')}
                            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition ${isDark ? 'bg-slate-800 text-red-400' : 'bg-slate-100 text-red-600'} hover:bg-red-500 hover:text-white`}>
