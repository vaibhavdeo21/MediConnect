...
const Navbar = () => {
  ...
  const navClass = isDoctor
    ? (theme === 'dark' ? "bg-slate-950 border-b border-cyan-500/20 shadow-2xl" : "bg-white border-b border-blue-100 shadow-md")
    : (isPremium ? "bg-slate-950 border-b border-yellow-500/20 shadow-2xl" : "bg-white shadow-md border-b border-slate-100");

  const logoClass = isDoctor
    ? (theme === 'dark' ? "text-cyan-400" : "text-blue-700")
    : (isPremium ? "text-yellow-500" : "text-primary"); 
    
  const linkClass = isDoctor
    ? (theme === 'dark' ? "text-slate-300 hover:text-white" : "text-slate-700 hover:text-blue-700")
    : (isPremium ? "text-slate-300 hover:text-white" : "text-slate-600 hover:text-primary");
        ...
        <div className="flex justify-between h-20">
          ...
          <div className="flex items-center">
            <Link to="/" className={`text-2xl font-serif font-bold ${logoClass} flex items-center gap-2 tracking-tight`}>
              <span className={`${isDoctor ? (theme === 'dark' ? 'bg-cyan-500' : 'bg-blue-700') : isPremium ? 'bg-yellow-500' : 'bg-primary text-white'} px-2 py-0.5 rounded shadow-lg text-white`}>MC</span>
              MediConnect
            </Link>
          </div>
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            {user ? (
              <div className="flex items-center gap-6">
                ...
                {isDoctor ? (
                  <>
                    <Link to="/my-appointments" className={`${linkClass} font-bold text-sm uppercase tracking-widest transition-colors flex items-center gap-2`}>
                      <Calendar className={`h-4 w-4 ${theme === 'dark' ? 'text-cyan-400' : 'text-blue-700'}`} /> Schedule
                    </Link>
                    <Link to="/wallet" className={`${linkClass} font-bold text-sm uppercase tracking-widest transition-colors flex items-center gap-2`}>
                      <Wallet className={`h-4 w-4 ${theme === 'dark' ? 'text-cyan-400' : 'text-blue-700'}`} /> Earnings
                    </Link>
                  </>
                ) : (
                  <>
                ...
                <button 
                  onClick={toggleTheme}
                  className={`p-2 rounded-xl transition-all border ${theme === 'dark' ? 'bg-slate-900 text-yellow-500 border-white/10' : 'bg-slate-100 text-blue-700 border-blue-100'}`}
                >
            ...
            {user ? (
              <div className={`pt-6 border-t ${isDoctor ? 'border-cyan-500/10' : isPremium ? 'border-yellow-500/10' : 'border-slate-100'}`}>
                ...
                <div className="space-y-4 text-left">
                  ...
                  {/* Mobile Theme Toggle */}
                  <button onClick={toggleTheme} className={`flex items-center gap-3 font-bold text-sm ${linkClass}`}>
